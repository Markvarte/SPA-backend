...
namespace Task2_restAPI.Controllers
{
    ...
    public class FlatsController : ControllerBase
    {
        ...
        //GET: api/Flats/House/5
        [HttpGet("House/{houseId}")]
        public async Task<IEnumerable<Flat>> GetFlatsByHouseId(int houseId)
        {
            var rezult = await _context.Flats.AsQueryable().Where(flat => flat.HouseId.Equals(houseId)).ToListAsync();
        ...
        public async Task<ActionResult<Flat>> PostFlat(CreateFlatDTO flatDTO)
        {
            ...
            var flatDestination = iMapper.Map<CreateFlatDTO, Flat>(flatDTO);
            _context.Flats.Add(flatDestination);
            await _context.SaveChangesAsync();

            return Ok(flatDestination);
        }
